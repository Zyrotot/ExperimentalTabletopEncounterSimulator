...
Ability CreateRigidezRaivosa() {
  ...
  take_damage_effect.on_event =
      [](std::shared_ptr<combat::EventContext> base_context) {
        auto context = std::dynamic_pointer_cast<combat::DamageContext>(base_context);
        if (!context || !context->target) return;
  ...
  heal_effect.on_event = [](std::shared_ptr<combat::EventContext> base_context) {
    auto context = std::dynamic_pointer_cast<combat::HealContext>(base_context);
    if (!context || !context->target || context->is_temp_hp) return;
...
Ability CreateTrespassar() {
  ...
  effect.on_event = [](std::shared_ptr<combat::EventContext> base_context) {
    auto context = std::dynamic_pointer_cast<combat::DamageContext>(base_context);
    if (!context || !context->attacker || !context->target) return;
    if (!context->target->IsAlive()) {
      context->attacker->IncrementAbilityStack("Trespassar");
      // TODO(zyrotot): Trigger additional attack here - need to add attack queue system
    }
  };
