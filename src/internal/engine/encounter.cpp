...
Encounter::Encounter(std::vector<std::shared_ptr<entities::Entity>> side_a,
                     std::vector<std::shared_ptr<entities::Entity>> side_b)
    : side_a_(std::move(side_a)), side_b_(std::move(side_b)) {
}

int Encounter::FindSideOf(const entities::Entity* entity) const {
...
std::vector<std::shared_ptr<entities::Entity>> Encounter::GetLivingEnemiesOf(
    const entities::Entity* entity) const {
  int side = FindSideOf(entity);
  const auto& enemies = (side == 0) ? side_b_ : side_a_;

  std::vector<std::shared_ptr<entities::Entity>> living;
...
std::shared_ptr<entities::Entity> Encounter::GetFirstLivingEnemyOf(
    const entities::Entity* entity) const {
  auto living = GetLivingEnemiesOf(entity);
  return living.empty() ? nullptr : living.front();
}

std::vector<std::shared_ptr<entities::Entity>> Encounter::GetLivingAlliesOf(
    const entities::Entity* entity) const {
  int side = FindSideOf(entity);
...
const std::vector<std::shared_ptr<entities::Entity>>& Encounter::GetSideA()
    const {
  return side_a_;
}

const std::vector<std::shared_ptr<entities::Entity>>& Encounter::GetSideB()
    const {
  return side_b_;
}
